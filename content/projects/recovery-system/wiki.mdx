...
At a high level, the Recovery System is the board that decides when the rocket is allowed to deploy recovery. It combines redundant compute, hardware voting, safing interfaces, local sensing, protected power conversion, and ten high-current ignition channels on a single six-layer PCB.
...
Two protected battery feeds enter independent power paths before supplying the board's deployment and logic rails. Four buck converters generate the required 24 V and low-voltage domains.

Three STM32 control domains independently process local sensor data and receive flight-computer commands. Their deployment commands pass through discrete voting logic before reaching the driver banks. Hardware RBF, ESTOP, abort, and software-enable paths provide multiple independent mechanisms for preventing an unintended deployment.
